...
import { Calendar, Clock, Video, Phone, MapPin, User, CheckCircle, ExternalLink, Users } from "lucide-react"
...
export default function InterviewsPage() {
  ...
  const allInterviews = candidates
    .flatMap((candidate) =>
      (candidate.interviews || []).map((interview) => ({
        ...interview,
        candidate,
        jobTitle: jobRequisitions.find((j) => j.id === candidate.jobRequisitionId)?.title || "Unknown",
        departmentName: jobRequisitions.find((j) => j.id === candidate.jobRequisitionId)?.organizationalUnitName || "",
      })),
    )
  ...
  const upcomingInterviews = allInterviews.filter(
    (i) => i.status === "scheduled" && new Date(i.scheduledAt).toISOString().split("T")[0] >= today,
  )
  ...
  const getInterviewerNames = (interviewerIds: string[], interviewerNamesArr?: string[]) => {
    if (interviewerNamesArr && interviewerNamesArr.length > 0) {
      return interviewerNamesArr.join(", ")
    }
    return interviewerIds.map((id) => employees.find((e) => e.id === id)?.fullName || "Unknown").join(", ")
  }
  ...
  return (
    <AdminLayout title="Interviews" subtitle="Manage interview schedules, calendar and meeting links">
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl text-primary">{todayInterviews.length}</CardTitle>
            </CardHeader>
          </Card>
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              <CardDescription>This Week</CardDescription>
              <CardTitle className="text-2xl text-warning">{thisWeekInterviews.length}</CardTitle>
            </CardHeader>
          </Card>
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              <CardDescription>All Upcoming</CardDescription>
              <CardTitle className="text-2xl">{upcomingInterviews.length}</CardTitle>
            </CardHeader>
          </Card>
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              <CardDescription>Completed</CardDescription>
              <CardTitle className="text-2xl text-success">{completedInterviews.length}</CardTitle>
            </CardHeader>
          </Card>
        </div>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <Badge className={cn("text-xs", getInterviewTypeColor(interview.type))}>
                    ...
                    <div className="text-right">
                      ...
                      <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                        ...
                        {getInterviewerNames(interview.interviewers, interview.interviewerNames)}
                      </div>
                    </div>
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            ...
            <CardDescription>All scheduled interviews this week</CardDescription>
          </CardHeader>
          <CardContent>
            {thisWeekInterviews.length === 0 ? (
              ...
              <div className="space-y-3">
                {thisWeekInterviews.map((interview) => {
                  ...
                  const isToday = interviewDate.toISOString().split("T")[0] === today
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="text-center min-w-[60px]">
                          <p className="text-xs text-muted-foreground uppercase">
                            {interviewDate.toLocaleDateString("en-US", { weekday: "short" })}
                          </p>
                          <p className="text-2xl font-bold">{interviewDate.getDate()}</p>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <Badge className={cn("capitalize", getInterviewTypeColor(interview.type))}>
                          {interview.type}
                        </Badge>
                        {interview.meetingLink && (
        ...
        {completedInterviews.length > 0 && (
          <Card className="bg-card border-border">
            <CardHeader>
              ...
              <CardDescription>Past interviews with feedback and recommendations</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Interviewers: {getInterviewerNames(interview.interviewers, interview.interviewerNames)}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
