...
import { Search, Filter, Fingerprint, Upload, AlertCircle, CheckCircle2, X } from "lucide-react"
...
export default function AttendanceRecordsPage() {
  ...
  const absentCount = canViewAllRecords ? employees.length - todayRecords.length : 0

  return (
    <AdminLayout
      title={canViewAllRecords ? "Attendance Management" : "My Attendance"}
      subtitle={
        canViewAllRecords ? "Monitor real-time attendance and view historical records" : "View your attendance history"
      }
    >
      <div className="space-y-6">
        <Card className="bg-blue-500/10 border-blue-500/20">
          <CardContent className="py-3">
            <div className="flex items-center gap-2 text-sm text-blue-600 dark:text-blue-400">
              ...
              <span>Attendance data is automatically synced from the fingerprint check-in/check-out machine</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Tabs defaultValue="history" className="w-full">
          <TabsList>
            ...
            {canViewAllRecords && <TabsTrigger value="import">Import Data</TabsTrigger>}
          </TabsList>
              ...
              <div className="grid gap-4 md:grid-cols-4">
                <Card className="bg-card border-border">
                  <CardHeader className="pb-2">
                    ...
                    <CardTitle className="text-2xl text-success">{presentCount}</CardTitle>
                  </CardHeader>
                </Card>
                <Card className="bg-card border-border">
                  <CardHeader className="pb-2">
                    <CardDescription>Late Today</CardDescription>
                    <CardTitle className="text-2xl text-warning">{lateCount}</CardTitle>
                  </CardHeader>
                </Card>
                <Card className="bg-card border-border">
                  <CardHeader className="pb-2">
                    <CardDescription>Not Checked In</CardDescription>
                    <CardTitle className="text-2xl text-destructive">{absentCount}</CardTitle>
                  </CardHeader>
                </Card>
                <Card className="bg-card border-border">
                  <CardHeader className="pb-2">
                    <CardDescription>Total Records</CardDescription>
                    <CardTitle className="text-2xl">{baseRecords.length}</CardTitle>
                  </CardHeader>
                </Card>
              </div>
            )}
                      ...
                      {filteredRecords.length > 0 ? (
                        filteredRecords.map((record) => (
                          <TableRow key={record.id}>
                            {canViewAllRecords && <TableCell className="font-medium">{record.employeeName}</TableCell>}
                            ...
                            <TableCell>
                              ...
                              {record.overtime && (
                                <span className="text-xs text-success ml-1">(+{record.overtime}h OT)</span>
                              )}
                            </TableCell>
                        ...
                        <TableRow>
                          <TableCell colSpan={canViewAllRecords ? 6 : 5} className="text-center py-8">
          ...
          {canViewAllRecords && (
            <TabsContent value="import" className="space-y-6">
              <Card className="bg-card border-border">
                <CardHeader>
                  <CardTitle className="text-base">Import Fingerprint Data</CardTitle>
                  <CardDescription>Upload attendance file from your fingerprint machine</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      ...
                      {uploadingFile ? `Uploading ${uploadingFile}...` : "Upload Attendance File"}
                    </Button>
                    <span className="text-sm text-muted-foreground">CSV, XLSX, JSON, or TXT format</span>
                  </div>

                  <div className="text-xs text-muted-foreground space-y-1 p-3 bg-secondary/30 rounded-lg">
                    <p className="font-medium text-foreground/80">Supported formats:</p>
                    <ul className="list-disc list-inside space-y-0.5">
                      <li>CSV: employee_id, date, check_in_time, check_out_time</li>
