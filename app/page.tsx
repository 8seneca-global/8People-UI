...
function AdminDashboard() {
  ...
  const vacantPositions = positions.filter((p) => p.hiringStatus === "vacant" || p.hiringStatus === "hiring").length
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard
            ...
            icon={CalendarDays}
            trend={pendingLeaves > 0 ? { value: pendingLeaves, isPositive: false } : undefined}
          />
          <StatCard
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>{pendingLeaves} requests need your attention</CardDescription>
                    </div>
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent>
                <div className="grid grid-cols-5 gap-2">
                  {[
                    { label: "New", count: candidates.filter((c) => c.stage === "new").length, color: "bg-blue-500" },
                    {
                      label: "Screening",
                    ...
                    {
                      label: "Interview",
                      count: candidates.filter((c) => c.stage === "interviewing").length,
                      color: "bg-yellow-500",
                    },
                    {
                      label: "Offering",
                      count: candidates.filter((c) => c.stage === "offering").length,
                      color: "bg-cyan-500",
                    },
                    {
                      label: "Hired",
                    ...
                    <div key={stage.label} className="text-center">
                      <div className={`${stage.color} text-primary-foreground rounded-md py-2 text-xl font-bold`}>
                        {stage.count}
                      </div>
            ...
            <Card className="bg-card border-border">
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium text-success">{presentToday}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-medium">{todayAttendance.filter((r) => r.status === "on_leave").length}</span>
                </div>
                <Progress value={(presentToday / activeCount) * 100} className="h-2 mt-2" />
                <p className="text-xs text-muted-foreground text-center">
                  {Math.round((presentToday / activeCount) * 100) || 0}% attendance rate
                </p>
              </CardContent>
            </Card>
                ...
                <CardContent>
                  <div className="space-y-2">
                    {notifications
                      ...
                      .slice(0, 3)
                      .map((notif) => (
                        <div key={notif.id} className="text-sm rounded-md bg-secondary/50 p-2">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="bg-card border-border">
              <CardHeader>
                ...
                <CardDescription>Track new employees completing onboarding</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {employees
                  ...
                  .slice(0, 4)
                  .map((emp) => (
                    <div key={emp.id} className="flex items-center gap-4 rounded-lg border border-border p-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/20">
                        <span className="text-sm font-medium text-primary">{getInitials(emp.fullName)}</span>
                      </div>
                      <div className="flex-1">
                        <p className="font-medium text-card-foreground">{emp.fullName || "Unknown"}</p>
                        <p className="text-xs text-muted-foreground">
                          {emp.positionTitle || "No title"} - {emp.organizationalUnitName || "No unit"}
                        </p>
                      </div>
                      <div className="text-right">
                ...
                {employees.filter((e) => e.status === "pending").length === 0 && (
                  <p className="text-center text-muted-foreground py-4">No pending onboarding</p>
                )}
              </CardContent>
            </Card>
                  ...
                  .map((job) => (
                    <div key={job.id} className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{job.organizationalUnitName}</p>
                      </div>
                      <Badge variant="outline">{job.openings - job.hired} open</Badge>
                    </div>
                  ))}
...
function EmployeeDashboard() {
  const { leaveBalances, leaveRequests, attendanceRecords, leaveTypes, notifications, employees } = useStore()

  // For demo, using the first active employee
  const currentEmployee = employees.find((e) => e.status === "active") || employees[0]
  ...
  const today = new Date().toISOString().split("T")[0]
  const todayRecord = attendanceRecords.find((r) => r.employeeId === currentEmployeeId && r.date === today)
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                {currentEmployee?.positionTitle} - {currentEmployee?.organizationalUnitName}
              </p>
            </div>
            <Button asChild>
        ...
        <StatCard
          ...
          value={
            currentEmployee?.startDate
              ? Math.floor(
                (new Date().getTime() - new Date(currentEmployee.startDate).getTime()) / (1000 * 60 * 60 * 24),
              )
              : 0
          }
          description={
            currentEmployee?.startDate ? `Since ${new Date(currentEmployee.startDate).toLocaleDateString()}` : ""
          }
          icon={Calendar}
        />
        <StatCard
          ...
          icon={CalendarDays}
        />
        <StatCard title="Pending Requests" value={pendingRequests} description="Leave requests" icon={Clock} />
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="bg-card border-border">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-card-foreground">Leave Balance</CardTitle>
                  <CardDescription>Your remaining leave days for 2026</CardDescription>
                </div>
                <Button asChild>
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-4">
                {myBalances.slice(0, 3).map((balance) => {
                  ...
                  return (
                    <div key={balance.leaveTypeId} className="text-center p-3 rounded-lg bg-secondary/50">
                      <p className="text-2xl font-bold">{balance.available}</p>
                      <p className="text-xs text-muted-foreground">{leaveType?.name}</p>
                      <p className="text-xs text-muted-foreground">
...
export default function DashboardPage() {
  ...
  const getDashboardTitle = () => {
    switch (currentRole) {
      case "admin":
        return { title: "Admin Dashboard", subtitle: "System overview and management" }
      case "hr":
        return { title: "HR Dashboard", subtitle: "Employee and onboarding management" }
      case "employee":
