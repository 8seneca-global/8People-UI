...
export function DivisionView({ node, onMerge, onDelete }: DivisionViewProps) {
    ...
    const fteCount = Math.floor(node.employeeCount * 0.85)
                        ...
                        <div>
                            ...
                            <div className="rounded-md border">
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        {(() => {
                                            const flattenPositionsInTreeOrder = (currentNode: OrgUnitNode, parentNode?: OrgUnitNode): Array<{ position: any, unitNode: OrgUnitNode, parentUnitNode?: OrgUnitNode }> => {
                                                const result: Array<{ position: any, unitNode: OrgUnitNode, parentUnitNode?: OrgUnitNode }> = []
                                                currentNode.positions.forEach(pos => {
                                                    result.push({ position: pos, unitNode: currentNode, parentUnitNode: parentNode })
                                                })
                                                currentNode.children.forEach(child => {
                                                    result.push(...flattenPositionsInTreeOrder(child, currentNode))
                                                })
                                                return result
                                            }
                                            ...
                                            return filteredPositions.map(({ position: pos, unitNode, parentUnitNode }) => {
                                                const incumbent = employees.find(e => e.id === pos.incumbentId)
                                                const status = pos.status || (incumbent ? "Filled" : "Vacant")
                                                const reportsTo = pos.reportsToId ? employees.find(e => e.id === pos.reportsToId) : null

                                                // Determine Department and Team based on hierarchy
                                                let departmentName = "—"
                                                let teamName = "—"

                                                if (unitNode.type === "team") {
                                                    // Position is in a team
                                                    teamName = unitNode.name
                                                    departmentName = parentUnitNode?.name || "—"
                                                } else if (unitNode.type === "department") {
                                                    // Position is directly in a department
                                                    departmentName = unitNode.name
                                                    teamName = "—"
                                                } else {
                                                    // Position is directly in the division
                                                    departmentName = "—"
                                                    teamName = "—"
                                                }
                                                ...
                                                return (
                                                    <tr key={pos.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                                                        ...
                                                        <td className="p-3 text-sm text-muted-foreground">
                                                            {departmentName === "—" ? <span className="text-muted-foreground/40">—</span> : departmentName}
                                                        </td>
                                                        <td className="p-3 text-sm text-muted-foreground">
