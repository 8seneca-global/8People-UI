...
import { Users, UserPlus, Settings, MoreHorizontal, Building, Trash2, MoveRight, Merge } from "lucide-react"
                        ...
                        <div>
                            ...
                            <div className="rounded-md border">
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        {(() => {
                                            const allPositions = node.positions.map(pos => ({ position: pos }))
                                            const filteredPositions = allPositions.filter(({ position: pos }) => {
                                            ...
                                            return filteredPositions.map(({ position: pos }) => {
                                                const incumbent = employees.find(e => e.id === pos.incumbentId)
                                                const status = pos.status || (incumbent ? "Filled" : "Vacant")
                                                ...
                                                return (
                                                    <tr key={pos.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                                                        ...
                                                        <td className="p-3 text-sm text-muted-foreground">{node.name}</td>
                                                        <td className="p-3">
                                                            {incumbent ? (
