...
export type { Employee, OrganizationalUnit, JobClassification, Position };
import type {
  Role,
  FeaturePermission,
  ModuleConfig,
...
export const useStore = create<StoreState>((set, get) => ({
  ...
  addJobClassification: (job) =>
    set((state) => ({
      jobClassifications: [
        ...state.jobClassifications,
        { ...job, id: `C-${Date.now()}` },
      ],
    })),
  updateJobClassification: (id, job) =>
    set((state) => ({
      jobClassifications: state.jobClassifications.map((j) =>
        j.id === id ? { ...j, ...job } : j,
      ),
    })),
  ...
  addEmployee: (employee) =>
    set((state) => ({
      employees: [
        ...state.employees,
        { ...employee, id: `P-${Date.now()}`, auditLog: [] },
      ],
    })),
  ...
  updateEmployeeSelfService: (id, field, value, changedByName) =>
    set((state) => {
      ...
      const oldValue = String(
        (employee as unknown as Record<string, unknown>)[field] || "",
      );
  ...
  updateLeaveRequest: (id, request) =>
    set((state) => ({
      leaveRequests: state.leaveRequests.map((r) =>
        r.id === id
          ? { ...r, ...request, updatedAt: new Date().toISOString() }
          : r,
      ),
    })),
